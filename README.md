...
> 间隔 45 分钟的两轮全表测出来 PRE 的长提示从 1.25k 掉到 1.22k（−2.4%）、
> decode 从 146.9 掉到 145.4（−1.0%），而**三版同幅下移**，所以表里的百分比稳定、绝对值不稳定。
> 结论：**拿这张表比版本，不要拿它比时段**；要跟别的时段对账，当场把两臂都跑一遍。
...
|---|---|
| **项目最后提交** | 只有两条 bf16 路（小 T 的 tile 路 + 大 T 的 MMA 路）。**它那时还没有 s8 档** |
| **沈三殊** | **prefill 多一条 int8 路**（激活按 token 做 absmax 量化）；decode 仍走 bf16 |
| **最新提交** | 同沈三殊 |
...
> **注意 MTP 的接受率**：项目最后提交与沈三殊都是 67.6%，最新提交是 **66.3%**。
...
不分这道门时整臂 −1%，加了之后 +1.3~1.4%。六夹具 +0.2~1.8%、接受长度无一下降，
**PPL 两臂逐位相同**（prefill 根本不切，所以上表两个数不受影响）。同样不是逐位相同，
