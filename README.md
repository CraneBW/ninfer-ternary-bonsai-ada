...
| 项 | 取值 |
|---|---|
| KV 精度 | **`bf16`**（三版都认的档位；`rk4v4` 是部署默认，但沈三殊版的 **CLI 不认它**，只有 serve 认） |
...
**一句话**：长提示已经追平沈三殊版，其余 prefill 场景全面反超，decode 保持 18% 领先。
...
`./three-way-long.sh`，KV = bf16，真交替 3 轮中位数：

| 提示规模 | 精度 | 项目最后提交 | 沈三殊 | **最新提交** |
|---|---|---:|---:|---:|
| **28,199**<br><sub>`--max-context 32768`</sub> | 各自默认 | 1.18k <sub>(bf16)</sub> | 1.77k <sub>(int8)</sub> | **1.77k** <sub>(int8)</sub> |
| | **同为 bf16** | 1.18k | **873** | **1.19k** |
| **61,636**<br><sub>`--max-context 65536`</sub> | 各自默认 | 1.01k <sub>(bf16)</sub> | 1.44k <sub>(int8)</sub> | **1.43k** <sub>(int8)</sub> |
| | **同为 bf16** | 1.01k | **763** | **1.01k** |

三个结论，**两个长度上都成立**：

1. **剥掉精度差异后，本机的 bf16 prefill 内核快沈三殊版 36%（28k）/ 33%（64k）**。
   沈三殊版的 prefill 优势**全部**来自它的 int8 档——这一条在 T=1.7k、28k、62k 上都成立。
2. **加回精度，两者持平**（1.77k 对 1.77k；1.43k 对 1.44k）：int8 档本仓库已经移植过来，
   那条优势就不再是它的了。
3. **项目最后提交与最新提交在 bf16 下持平**（1.18k / 1.18k；1.01k / 1.01k）。这符合预期——
   **本轮改的两处（prefill token tile 的 T≤64 分档、prompt 13~35 的档位空洞）都不落在长 T 上**。
   长提示这一格本轮没有变快，变快的是短提示；它要做的是"追平"，而它追平了。
...
**什么时候该换 KV 档**：decode 每 token 读 6.70 GiB 权重，而 KV 读随上下文线性增长。
两者相等的位置（16 层全注意力 × 4 KV 头）：

| KV 档 | 每 token | KV 读 = 权重读的上下文 |
|---|---:|---:|
| `bf16` | 64 KiB | **≈ 110k** |
| `fp8` / `int8` | 32.3 / 33.0 KiB | ≈ 217k / 213k |
| **`rk4v4`** | **17.0 KiB** | **≈ 413k（超出原生上限，永不发生）** |

这张表就是 `rk4v4` 存在的定量理由：它把"KV 读盖过权重"的点从 110k 推到上限之外。
同时也说明 **262,144 那一行只有 `rk4v4` 两臂跑得起来**，`bf16`/`fp8`/`int8` 会先 OOM。
...
> **★ 这两个数是 `fp8` KV 的数。** `ninfer-perplexity` 的 KV 默认值是 `fp8`（源码写死，
> 见 `apps/perplexity/main.cpp`），**而 `ninfer` CLI 的默认是 `bf16`**（`apps/cli/options.h`）。
> 两者都是各自的默认，但**协议不同、数不要混**——§1.1 的性能表是 bf16 KV，这一节是 fp8 KV。
...
|---|---:|---:|---:|---|
| **`bf16`** | **9.688451** | — | 64.00 KiB | CLI 默认。无损基准 |
| `int8` | 9.691663 | +0.033% | 33.00 KiB | **比 `fp8` 更准** |
| `fp8` | 9.693396 | +0.051% | 32.25 KiB | `ninfer-perplexity` 的默认 |
| `rk4v4` | 9.702774 | **+0.148%** | **17.00 KiB** | 长上下文的唯一档位 |
...
1. **`int8` 比 `fp8` 准**（+0.033% vs +0.051%），字节只多 2.3%。原因是编码方式：
   ...
   每 256 维才一个 scale。**逐元素精度差约 6 倍。** 所以 `int8` 是比 `fp8` 更好的默认档。
...
> （不是更低）。长口径（`--context 65536 --stride 32768`，上游文档的 KV 对照推荐口径）
> 是下一步，**未做**。
...
**`--draft-tokens` 的最优点随任务移动**（2026-09-26 实测，三个夹具各拟合一次，
模型参数几乎重合：T0 ≈ 15.83 ms、每个 draft 步 c ≈ 1.94 ms）：

| 任务 | 每位置存活率 | 最优 K | K=3 → K\* |
|---|---:|---:|---:|
| `en-code`（写代码，低可预测性） | ≈0.76 | **4** | 136.5 → **143.3（+5%）** |
| 数数 / 结构化输出 | ≈0.95 | **7** | 178.7 → ~240（+34%） |
| 模板复读（上界） | ≈0.995 | **>7** | 184.2 → ~272（+48%） |

**接受率不是常数，它随位置衰减，而衰减速度是任务的属性。** 所以"draft 4/5 只会更慢"
只对低可预测性任务成立。**当前 K 被四个常量锁在 ≤5，硬上界是 K≤7**（`T=K+1≤8`，
再高会掉出 `small_t`，验证轮从 13.3 ms 跳到 40 ms）。
