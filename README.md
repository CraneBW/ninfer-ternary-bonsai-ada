...
| 项 | 取值 |
|---|---|
| KV 精度 | **`bf16`**（三版都认的档位；`rk4v4` 是部署默认，但正式版的 **CLI 不认它**，只有 serve 认） |
...
| 版本 | 它跑的是什么精度 |
|---|---|
| **预览版改良** | 纯 **bfloat16** —— 它那时还没有 int8 档 |
| **正式版** | **int8 档默认开**（激活量化到 int8；权重始终是三元 2.125 bit） |
| **本机当前版** | **int8 档默认开**（同上） |

所以「各自默认」那张表**不是同精度比较**：正式版与本机当前版吃 int8，预览版吃 bf16。
这不是掩饰——**它正是差距的主要来源之一**，而且是个真实的部署差异：上游把 int8 档默认设为开。

**要同精度比较，三版都关掉 int8：**

```bash
NINFER_TERNARY_S8=0 <binary> <model> ...      # 预览版没有这个开关，设了也无害
```
...
| 提示规模 | 预览版改良<br>(bf16) | 正式版<br>(int8) | **本机当前版**<br>(int8) | 对预览版 | 对正式版 |
|---|---:|---:|---:|---:|---:|
...
§1.1 那张表是**各自默认精度**——正式版与本机当前版吃 int8，预览版吃 bf16。
把 int8 关掉（`NINFER_TERNARY_S8=0`）才能看出**内核本身**谁强。

`long-30k-doc.json`（**28,199 token**），KV = bf16，`--max-context 32768`：

| 精度 | 预览版改良 | 正式版 | **本机当前版** |
|---|---:|---:|---:|
| 各自默认 | 1.18k <sub>(bf16)</sub> | 1.77k <sub>(int8)</sub> | **1.77k** <sub>(int8)</sub> |
| **同为 bf16** | 1.18k | **873** | **1.19k** |

（同 §1.1 的真交替 3 轮中位数，`./three-way-long.sh`。）

三个结论：

1. **剥掉精度差异后，本机的 bf16 prefill 内核在长提示上快正式版 36%**（1.19k 对 873）。
   正式版的 prefill 优势**全部**来自它的 int8 档——这一条在 1.7k 和 28k 上都成立。
2. **加回精度，两者持平**（1.77k 对 1.77k）：int8 档本仓库已经移植过来，那条优势就不再是它的了。
3. **预览版与当前版在 bf16 下持平**（都是 1.18k）。这符合预期——**本轮改的两处
   （prefill token tile 的 T≤64 分档、prompt 13~35 的档位空洞）都不落在 28k 这个 T 上**。
   长提示这一格本轮没有变快，变快的是短提示；长提示这一格要做的是"追平"，而它追平了。

> 超过 64k 的三方对比用 CLI 做不了：128k 上下文在 16 GB 上只有 4-bit KV 放得下，
> 而**正式版的 CLI 不认 `rk4v4`**（只有它的 serve 认）。要做得改用 serve，口径需要单独设计。
