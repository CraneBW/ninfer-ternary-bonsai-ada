...
|---|---:|---:|---:|---:|---:|
| 长（T=1759） | 1.25k | 1.96k | **2.54k** | **+103%** | **+30%** |
| 中（T=123） | 791.2 | 1.32k | **1.59k** | **+101%** | **+21%** |
| 短（T=38） | 281.0 | 633.9 | **755.5** | **+169%** | **+19%** |
| 极小（T=15） | 144.8 | 331.9 | **386.6** | **+167%** | +16% |

**Decode**（tok/s，`en-code.json`，300 token）：

| 场景 | 项目最后提交 | 沈三殊 | **最新提交** | 对项目最后提交 | 对沈三殊 |
|---|---:|---:|---:|---:|---:|
| MTP draft 3 | 146.9 | 124.3 | **146.4** | −0.3% | **+18%** |
| └ 轮净耗时（ms） | 20.56 | 24.30 | **20.36** | **−1.0%** | **−16%** |
| 无投机 | 64.6 | 57.8 | **64.6** | 持平 | **+12%** |
...
> **decode 那一行的来历**：最新提交的验证路（T ≤ 8 的 small_t）**按输出行数逐形状选 row block**
> （`n ≤ 5120` 用 16，否则 32），因为响应是非单调的、一个全局值不可能对——`NINFER_TERNARY_SMALL_T_ROWS`
> 可以覆盖。真交替双向 A/B 都是 **+1.8%**（132.3 → 134.7 tok/s，200 token 与 300 token 两个口径），
> 接受率、`accepted by pos` 直方图、生成文本全部不变。

**一句话**：长提示从"追平"变成**反超 30%**，其余 prefill 场景全面领先，
decode 保持 18% 领先（接受率那一格的差别见上）。
